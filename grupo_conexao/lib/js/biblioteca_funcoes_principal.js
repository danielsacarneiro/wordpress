...
CD_CAMPO_SUBSTITUICAO = "[[*]]"; // Deve ser igual � constante br.gov.pe.sefaz.sfi.util.Constantes.CD_CAMPO_SUBSTITUICAO
CD_CAMPO_SUBSTITUICAO_MSGUSR = "#?#"; // Deve ser igual � constante br.gov.pe.sefaz.sfi.util.Constantes.CD_CAMPO_SUBSTITUICAO_MSGUSR
...
ID_REQ_IN_LIMPAR_UNIDADE_ALOCACAO_SELECIONADA = "in_limpar_unidade_alocacao_selecionada"; // Deve ser igual � constante br.gov.pe.sefaz.sfi.util.web.ProcessadorRequisicao.ID_REQ_IN_LIMPAR_UNIDADE_ALOCACAO_SELECIONADA
...
// Integra��o com a biblioteca jsTrace.js
// Para utilizar, basta importar jsTrace.js na JSP e invocar a fun��o trace em qualquer ponto do c�digo.
...
// Confirma a execu��o de uma opera��o. Se pTexto for nulo, ser� exibida uma mensagem padr�o de confirma��o.
...
// Verifica se um elemento HTML � v�lido
...
function getQtElementos(pIdElemento) {
	...
	// n�o existe nenhum elemento com o id informado
	if (r == null) {
		return 0;
	} else {
		if (r.length == null) {
...
function _abrirJanelaAuxiliarMovel(pUrlJanela, pNmJanela) {
	...
	janelaAuxiliar.onload = function() {
		if (_janelaAuxiliarMovelElementos !== null) {
			// Tratamento para o caso de refresh da p�gina.
			return;
		}
		
		_janelaAuxiliarMovelElementos = [];
		
		// � necess�rio esconder todos os elementos porque o navegador nativo do Android 2.3
		// n�o suporta definir "overflow" (utilizado no document.documentElement).
...
function ocultarMensagemAguarde() {
	...
	if (_nmBotaoAtivado != "") {
		// Habilita bot�o que iniciou transa��o
	...
	// Habilita bot�es de a��o da tela de mensagem ao usu�rio
	elementos = document.forms.item(0).elements;
	for (var i = 0; i < elementos.length; i++) {
...
// Indica se o formul�rio j� foi submetido. Ap�s um history.back() est� vari�vel � sempre false.
var _inFormSubmetido = false;

// Nome e t�tulo do bot�o acionado no formul�rio.
var _nmBotaoAtivado;
var _nmTitleBotaoAtivado;

// Indica se algum bot�o que inicia uma transa��o foi ativado (Confirmar, etc...).
var _inBotaoOperacaoAtivado = false;

// Chamada quando se clica em qualquer bot�o do framework
// Se for o bot�o Confirmar, invoca setBotaoOperacaoAtivado
...
// Verifica se existe no formul�rio um bot�o Confirmar
...
// Deve ser chamada dentro da fun��o JavaScript associada ao evento onClick de um bot�o que 
// dispara uma transa��o (processarXxxxxx), antes de chamar a fun��o submeterFormulario
function setBotaoOperacaoAtivado(pBotao) {
	_inBotaoOperacaoAtivado = true;
}

// Deve ser chamada dentro da fun��o JavaScript que submete o formul�rio para atualizar a p�gina, ap�s chamar a fun��o submeterFormulario
// Ex.: fun��o chamada pelo evento onchange de um select, fun��o chamada pelo evento onblur de um text
function setFormNaoSubmetido() {
	_inFormSubmetido = false;
}

// Deve ser chamada dentro da p�gina que � exibida quando um bot�o de opera��o � ativado e a opera��o � conclu�da com sucesso
...
// Deve ser chamada dentro da p�gina que � exibida quando um bot�o de opera��o � ativado e a opera��o � conclu�da sem sucesso
...
function submeterFormulario(pAcao, pEvento, pInNaoExibirMensagemAguarde) {
	...
	// Trata ressubmiss�o de formul�rios
	if (existeBotaoOperacaoConfirmar() || _inBotaoOperacaoAtivado) {
		...
		if (_inFormSubmetido) {
			if (_inBotaoOperacaoAtivado) {
				// Um bot�o de opera��o foi ativado mas a requisi��o foi interrompida pelo usu�rio.
			...
			if (campoInFormularioSubmetido != null) {
				...
				if (inFormularioSubmetido == CD_VERDADEIRO && _inBotaoOperacaoAtivado) {
					// O formul�rio j� foi submetido anteriormente e um bot�o de opera��o foi ativado na mesma p�gina.
					
					var inOperacaoSucesso = getCookie("sfi.in_operacao_sucesso");
					
					if (inOperacaoSucesso == CD_VERDADEIRO) {
						// A opera��o anterior foi conclu�da com sucesso.
...
// Encerra Sess�o (utilizada pelo GerenciadorMolduraCabecalho)
...
// Encerra Sess�o usando certificado digital (utilizada pelo GerenciadorMolduraCabecalho)
function encerrarSessaoComCertificado(pAcao) {
	if (solicitarConfirmacao(mensagemGlobal(15))) {
		submeterFormulario(pAcao, "");
		window.open("", "_parent"); // Exigida para fechar o browser no firefox e netscape, quando este n�o permite fechar o browser em suas configura��es locais.
		window.close();
	}
}

// Limpa a unidade de aloca��o selecionada e persistida em mem�ria, para quem tem mais de uma unidade de aloca��o
function limparUnidadeAlocacaoSelecionada(pAcao) {
	...
	if (solicitarConfirmacao(mensagemGlobal(cdMensagemGlobal))) {
	    ...
	    //Atribui��o dos atributos e seus valores.
...
// Retorna o primeiro campo do formul�rio, caso exista
			...
			if ((elementos.item(i).type == "text" || elementos.item(i).type == "password") && elementos.item(i).readOnly != true) {
				...
				if (elementos.item(i).length > 0) {
					if (elementos.item(i).length == 3 && elementos.item(i).options[1].text == "Ativo" && elementos.item(i).options[2].text == "Inativo") {
						// O SelectIndicadoAtivo deve reiniciar na op��o Ativo
...
// Fun��o para testar se uma janela pai � uma janela do e-Fisco
...
// Fun��o que indica se a aplica��o est� executando dentro do ambiente de teste do Selenium 
...
// Configura a pagina de acordo com resolu��o e navegador
function configurarPagina() {
	...
	if (isAmbienteDesenvolvimento()) {
		...
		if (campoIdContextoSessao == null) {
			exibirMensagem("ATEN��O: O campo oculto \"" + ID_REQ_ID_CONTEXTO_SESSAO + "\" n�o existe nesta jsp!");
		} else {
			if (campoIdContextoSessao.length > 1) {
				exibirMensagem("ATEN��O: Existem " + campoIdContextoSessao.length + " campos ocultos \"" + ID_REQ_ID_CONTEXTO_SESSAO + "\" nesta jsp!");
			}
		}
		...
		if (campoEvento == null) {
			exibirMensagem("ATEN��O: O campo oculto \"" + ID_REQ_EVENTO + "\" n�o existe nesta jsp!");
		} else {
			if (campoEvento.length > 1) {
				exibirMensagem("ATEN��O: Existem " + campoEvento.length + " campos ocultos \"" + ID_REQ_EVENTO + "\" nesta jsp!");
			}
		}
	}
	...
	// Se for uma tela de consulta e s� tiver um registro no grid, seleciona
	filtro = document.getElementById("div_filtro");
	if (filtro != null) {
	...
	elementos = document.forms.item(0).elements;
	for (var i = 0; i < elementos.length; i++) {
		// Teste para evitar utiliza��o das teclas de atalho do framework
		if (elementos.item(i).type == "button") {
			if (!isBotaoFramework(elementos.item(i))) {
				if (isTeclaAtalhoFramework(elementos.item(i).accessKey)) {
					if (isAmbienteDesenvolvimento()) {
						exibirMensagem("ATEN��O: A tecla de atalho (" + elementos.item(i).accessKey + ") do bot�o \"" + elementos.item(i).name + "\" j� est� associada a um bot�o do framework!");
					}
				}
			}
		}
			...
			if (isAmbienteDesenvolvimento()) {
				...
				if (colBody != null && colBody.length > 0) {
					...
					if (body.onload == null) {
						...
						exibirMensagem("ATEN��O: O campo \"" + elementos.item(i).name + "\" usa a classe CSS \"camporeadonly\" mas n�o est� definido como \"readOnly\" ou \"disabled\"!");
					}
				}
	        }
		}
	...
	// Verifica se existe alguma �ncora no formul�rio
	...
	// Seta o foco para o pr�ximo campo
	...
	if (campoNmCampoFocoAtual != null){
		if (campoNmCampoFocoAtual.value != "") {
			...
			if (campoFocoAtual != null) {
				...
				//Seta o campoNmCampoFocoAtual para null para que o foco n�o fique voltando sempre para o mesmo campo
...
//Busca o nome do pr�ximo campo para receber o foco
...
// Configurar o cabe�alho fixo para tabela de dados. Esta fun��o s� deve ser chamada em p�ginas que contenham uma tabelaDados bem formatada.
function configurarTabelaDadosCabecalhoFixo() {
	...
	$(window).bind("scroll", function() {
	    ...
	    // Se com o scrool, o cabe�alho deixou de aparecer, mostra o cabe�alho fixo
	    if (topoAtual >= topoTabela && $cabecalhoFixo.is(":hidden")) {
	        $cabecalhoFixo.show();
	    } else if (topoAtual < topoTabela) {
	    	// esconde o cabe�alho fixo
	        $cabecalhoFixo.hide();
	    }
	});
}
...
/*
 * Fun��es utilit�rias para programa��o OO.
 */
